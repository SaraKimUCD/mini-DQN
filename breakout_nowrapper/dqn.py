...
for episode in range(EPISODES):
    state_stack = []
    dqn._reset()
    full_path = str(pathfile) + r"\dqn_breakoutv4_" + str(episode)
    action = 1
    state = env.step(action)[0] # start new episode
    ...
    if episode % 5 == 0:
        ...
        Q_df.to_csv(str(pathfile)  +'/v4all_Q_' + datetime.today().strftime('%Y%m%d') +'.csv', mode='a', header = False, index= False)
        rewards_df.to_csv(str(pathfile) +'/v4all_rewards_' + datetime.today().strftime('%Y%m%d') +'.csv', mode='a',header = False, index= False)
        avgr_df.to_csv(str(pathfile) +'/v4all_avg_reward_' + datetime.today().strftime('%Y%m%d') +'.csv', mode='a',header = False, index= False)
